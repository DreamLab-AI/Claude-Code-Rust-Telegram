...
pub(super) async fn handle_tool_start(ctx: &HandlerContext, msg: &BridgeMessage) {
    ...
    let summary = if tool_input.is_null() {
        ...
        let rule_based = summarize_tool_action(tool_name, &tool_input);
        ctx.llm_summarizer
            .summarize(tool_name, &tool_input, &rule_based)
            .await
    };
...
pub(super) async fn handle_tool_result(ctx: &HandlerContext, msg: &BridgeMessage) {
    ...
    // Build human-readable summary line (with LLM fallback for generic ones)
    let action_summary = if tool_input.is_null() {
    ...
    let text = format!("\u{2705} {action_summary}\n    {result_summary}");

    // Cache the result for the Details button using toolUseId if available
    let tool_use_id = meta
        .tool_use_id()
        .map(|s| s.to_string())
        .unwrap_or_else(|| {
    ...
    if !tool_input.is_null() && tool_input.as_object().is_some_and(|o| !o.is_empty()) {
        // Store input+output in cache for Details button
        {
            let mut cache = ctx.tool_cache.write().await;
            cache.insert(
                tool_use_id.clone(),
